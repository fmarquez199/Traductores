...
token_show (Sym p s)
  ...
  | otherwise = "Error: Caracter inesperado " ++ (show s) ++ " en la fila " ++ row ++ ", columna " ++ column
  where posx   = posn_show p
        row    = posn_show_row p
        column = posn_show_column p
token_show (Var p s)
  ...
  | s == ">="        = "TkMayorIgual "    ++ posx
  | s == "\\/"       = "TkDisyuncion "    ++ posx
  | otherwise        = isChar s           ++ posx
  where posx = posn_show p
token_show (Int p s) = "TkNum(" ++ (show s) ++ ") " ++ (posn_show p)

posn_show (AlexPn _ row colum) = show row ++ " " ++ show colum

posn_show_row (AlexPn _ row colum) = show row

posn_show_column (AlexPn _ row colum) = show colum
